...
class ResearchExecutionService:
    ...
    async def run(self, question: str, top_k: int = DEFAULT_TOP_K) -> ExecutionResult:
        ...
        try:
            ...
            raise ResearchExecutionError(f"Search failed: {exc.message}") from None
        logger.info(
        ...
        try:
            ...
            raise ResearchExecutionError(f"Indexing failed: {exc.message}") from None
        ...
        try:
            ...
            raise ResearchExecutionError(f"Synthesis failed: {exc.message}") from None

        # 6. ONE report generation (Markdown + PDF) from the outcome above.
        try:
            result: ReportResult = self._reports.generate(outcome)
        except ReportServiceError as exc:
            raise ResearchExecutionError(f"Report generation failed: {exc.message}") from None
