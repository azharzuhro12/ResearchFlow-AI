...
- **GLM API key** dari Z.ai — satu-satunya dependensi berbayar
- Docker (cara termudah), **atau** Python 3.14 + Node.js 22 untuk menjalankan manual

### Cara Docker (rekomendasi)

```bash
# 1. Salin environment dan isi secret kamu
cp backend/.env.example backend/.env
#    edit backend/.env: isi GLM_API_KEY dan GLM_MODEL
...
| Layanan   | URL                        | Catatan                                        |
| --------- | -------------------------- | ---------------------------------------------- |
| Frontend  | http://localhost:3000      | Menunggu backend sehat sebelum start           |
| Backend   | http://localhost:8010      | Kontainer mendengarkan 8000; healthcheck `/health` |
| Swagger   | http://localhost:8010/docs | Dokumentasi API interaktif                     |

Selesai — buka http://localhost:3000 dan ketik pertanyaan riset pertamamu.

### Cara Manual (tanpa Docker)

```bash
git clone https://github.com/azharzuhro12/ResearchFlow-AI.git researchflow-ai
cd researchflow-ai
...
cp .env.example .env          # lalu edit: GLM_API_KEY, GLM_MODEL
...
> Port backend memang **8010** (8000/8001 umumnya sudah terpakai);
> frontend mengetahuinya lewat `NEXT_PUBLIC_API_BASE_URL`.

---

## Konfigurasi

### Backend — `backend/.env` (salin dari `.env.example`; di-gitignore)

| Variabel | Wajib | Keterangan |
| -------- | ----- | ---------- |
| `GLM_API_KEY` | ✅ | API key GLM kamu (secret khusus backend) |
| `GLM_MODEL` | ✅ | Model GLM yang tersedia di akunmu |
| `GLM_BASE_URL` | ⬜ | Default `https://api.z.ai/api/anthropic` (format Anthropic Messages — di sinilah kuota GLM Coding Plan berlaku) |
| `DISCORD_WEBHOOK_URL` | ⬜ | Placeholder ⇒ notifikasi mati. URL incoming-webhook Discord — secret backend, divalidasi ketat |
| `DATABASE_URL` | ⬜ | Default `sqlite:///<proyek>/data/researchflow.db` |
| `CORS_ORIGINS` | ⬜ | Default `http://localhost:3000` |
| `DEFAULT_SCHEDULE_TIMEZONE` | ⬜ | Default `Asia/Jakarta` (zona waktu IANA untuk jadwal baru) |
| `CHROMA_DIR` / `CHROMA_COLLECTION_NAME` | ⬜ | Lokasi/nama vector store |
| `EMBEDDING_MODEL_NAME` | ⬜ | Default `sentence-transformers/all-MiniLM-L6-v2` |

### Frontend — `frontend/.env.local` (di-gitignore)

| Variabel | Keterangan |
| -------- | ---------- |
| `NEXT_PUBLIC_API_BASE_URL` | Base URL backend, `http://localhost:8010` |

> Tidak ada secret yang memakai prefix `NEXT_PUBLIC_`. Frontend tidak
> menerima API key, tidak menerima URL webhook, tidak menerima internal
> apa pun — hanya base URL API publik.
